...
function initNav() {
  ...
  document.querySelectorAll('a[href^="#"]').forEach(a => {
    a.addEventListener('click', e => {
      ...
      if (h && h.length > 1) {
        e.preventDefault();
        if (lenis) lenis.scrollTo(document.querySelector(h), { offset: -64, duration: 1.4 });
...
function initMagneticCursor() {
  ...
  // Use left/top approach for quickTo to work as translate offsets
...
function initGSAPHero() {
  ...
  tl.from('.heyebrow', { y: 24, opacity: 0, duration: 0.7, ease: 'power3.out' })
    ...
    .from('.scrollhint', { opacity: 0, duration: 0.8, ease: 'power2.out' }, '-=0.2');
}
...
function initGSAPScrollAnimations() {
  ...
  gsap.utils.toArray('.rv').forEach(el => {
    el.classList.remove('rv'); // Remove CSS fallback
    gsap.from(el, {
  ...
  if (skcats.length) {
    gsap.from(skcats, {
      ...
      scrollTrigger: { trigger: '.skgrid', start: 'top 85%' }
    });
  }

  // Cert cards
  const cflips = gsap.utils.toArray('.cflip');
  if (cflips.length) {
    gsap.from(cflips, {
      y: 30, opacity: 0, stagger: 0.15, duration: 0.7, ease: 'power3.out',
      scrollTrigger: { trigger: '.cgrid', start: 'top 85%' }
    });
  }
  ...
  // Contact links
  gsap.utils.toArray('.clnk').forEach((el, i) => {
    gsap.from(el, {
      x: -20, opacity: 0, duration: 0.5, ease: 'power3.out', delay: i * 0.07,
      scrollTrigger: { trigger: el, start: 'top 90%' }
    });
  });
}
...
function initSkillBars() {
  document.querySelectorAll('.skcat').forEach(cat => {
    ScrollTrigger.create({
      ...
      once: true,
      onEnter: () => {
        cat.querySelectorAll('.sbf').forEach((b, i) => {
...
/* =====================================================
   ...
   ===================================================== */
function initTilt() {
  document.querySelectorAll('.pcard,.sc2,.rc,.ainfo-card').forEach(card => {
    let raf;
    ...
    card.addEventListener('mouseleave', () => {
      cancelAnimationFrame(raf);
      gsap.to(card, { rotateX: 0, rotateY: 0, scale: 1, duration: 0.6, ease: 'elastic.out(1, 0.7)' });
    });
  });
}
...
function initMagnetic() {
  if (RM || MOB) return;
  document.querySelectorAll('.btnp,.btns,.nbtn').forEach(btn => {
...
function initCmdk() {
  ...
  function exec(cmd) {
    ...
    if (cmd.h) {
      const target = document.querySelector(cmd.h);
      if (target) {
  ...
  function openPalette() { ov.classList.add('open'); inp.value = ''; render(''); setTimeout(() => inp.focus(), 50); document.body.style.overflow = 'hidden'; }
...
function toggleTheme() {
  ...
  document.getElementById('thbtn').textContent = e ? '☀' : '🌙';
}
...
/* =====================================================
   ...
   ===================================================== */
function initGitHub() {
  const fb = () => { const r = document.getElementById('ghrepos'), f = document.getElementById('ghfol'), s = document.getElementById('ghstars'); if (r) r.textContent = '6'; if (f) f.textContent = '—'; if (s) s.textContent = '—'; };
  (async () => {
    try {
...
function initKonami() {
  ...
  document.addEventListener('keydown', e => {
    if (e.key === CODE[idx]) { idx++; if (idx === CODE.length) { idx = 0; toast.textContent = '🎮 Achievement: Power User! You found it. Kamal writes 42+ tests. ✅'; toast.classList.add('show'); setTimeout(() => toast.classList.remove('show'), 5000); } } else idx = 0;
  });
}